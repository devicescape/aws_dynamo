...
*aws_dynamo_parse_describe_table_response(const char *response, int response_len)
{
	yajl_handle hand;
	yajl_status stat;
	struct ctx _ctx = { 0 };
	...
	if (_ctx.r == NULL) {
		Warnx("aws_dynamo_parse_describe_table_response: response alloc failed.");
		return NULL;
	}
	...
	if (stat != yajl_status_ok) {
		...
		Warnx("aws_dynamo_parse_describe_table_response: json parse failed, '%s'", (const char *)str);
		yajl_free_error(hand, str);
		yajl_free(hand);
		aws_dynamo_free_describe_table_response(_ctx.r);
		return NULL;
	}
...
struct aws_dynamo_describe_table_response *aws_dynamo_describe_table(struct
								 aws_handle
								 *aws, const char
								 *request)
{
	...
	if (response == NULL) {
		Warnx("aws_dynamo_describe_table: Failed to get response.");
		return NULL;
	}
...
void aws_dynamo_dump_describe_table_response(struct
					   aws_dynamo_describe_table_response *r)
{
#ifdef DEBUG_PARSER
	if (r == NULL) {
...
void aws_dynamo_free_describe_table_response(struct
					   aws_dynamo_describe_table_response *r)
{
	if (r == NULL) {
		return;
